 ...
 */
public class BindEmailFragment extends BaseFragment implements
		View.OnClickListener {
	private static final String TAG = "BindPhoneFragment";
	private static final String PHONE = "phone";
	private static BindEmailFragment fragment;
	...
	public static BindEmailFragment newInstance(int layoutId, String phone) {
		if (null == fragment) {
	...
	private boolean ckeckPhoneNum() {
		...
		if (TextUtils.isEmpty(emailOrPhone)) {
			isOK = false;
			Toast.makeText(getActivity(), "请输邮箱号码", Toast.LENGTH_SHORT).show();
		} else if (!StringUtils.isEmail(emailOrPhone)) {
			isOK = false;
	...
	private void GoSendCode() {
		...
		params.put("type", "6");//类型（1注册 2找回密码 3修改密码 4修改银行卡信息 5绑定手机/绑定邮箱）
	...
	private void iSellerBdEmailSd() {
		...
		if (ckeck(valicode)) {
			...
			HttpClient.iSellerBdEmailSd(handler, params);
		}
	}
